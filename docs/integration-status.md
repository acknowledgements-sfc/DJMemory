...
- Live verification still required: run `.build/DJMemory.app/Contents/MacOS/DJMemory --app-audio-probe <seconds> serato` and `rekordbox` without `DJMEMORY_FORCE_SCK_APP_AUDIO=1`; confirm meter + staging WAV + archive write.
